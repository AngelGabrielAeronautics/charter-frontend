...
import {
  ...
  resetActionStates,
  update,
} from "@/lib/state/assets/assets.slice";
...
const EditAssetDrawer: React.FC<CreateAssetProps> = ({
  visible,
  onClose,
  asset,
}) => {
  ...
  const { loading, error, success, assets } = useAppSelector(
    (state) => state.assets
  );
  ...
  const selectedAsset = assets.find((item) => item._id === asset);
  ...
  const handleSubmit = () => {
    if (authenticatedUser) {
      form.validateFields().then(() => {
        ...
        dispatch(update({ id: asset, payload }));
      });
    }
  };
  ...
  useEffect(() => {
    if (selectedAsset) {
      // Populate the form with the values of selectedAsset
      form.setFieldsValue({
        operatorId: selectedAsset.operatorId,
        registrationNumber: selectedAsset.registrationNumber,
        manufacturer: selectedAsset.manufacturer,
        yearOfManufacture: selectedAsset.yearOfManufacture,
        model: selectedAsset.model,
        seatingCapacity: selectedAsset.seatingCapacity,
        baggage_compartment_max_weight:
          selectedAsset.baggage_compartment_max_weight,
        baggageCompartmentMaxWeightUnits:
          selectedAsset.baggageCompartmentMaxWeightUnits,
        baggageType: selectedAsset.baggageType,
        length: selectedAsset.baggageCompartmentSize[0],
        width: selectedAsset.baggageCompartmentSize[1],
        height: selectedAsset.baggageCompartmentSize[2],
        cabinLength: selectedAsset.cabinLength,
        cabinWidth: selectedAsset.cabinWidth,
        cabinHeight: selectedAsset.cabinHeight,
        cargoCapacity: selectedAsset.cargoCapacity,
        cargoCapacityUnits: selectedAsset.cargoCapacityUnits,
        inflightServicePersonnel: selectedAsset.inflightServicePersonnel,
        minimumCockpitCrew: selectedAsset.minimumCockpitCrew,
        numberOfCrew: selectedAsset.numberOfCrew,
        powerPlant: selectedAsset.powerPlant,
        airConAvailable: selectedAsset.airConAvailable,
        smokingAllowed: selectedAsset.smokingAllowed,
        hasWashCloset: selectedAsset.hasWashCloset,
        cabinPressure: selectedAsset.cabinPressure,
        petsAllowed: selectedAsset.petsAllowed,
        pressurized: selectedAsset.pressurized,
        heated: selectedAsset.heated,
        apu: selectedAsset.apu,
        // lastRefurbishmentDate: selectedAsset.lastRefurbishmentDate,
      });
    }
  }, [selectedAsset, form]);
